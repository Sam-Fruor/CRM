...
import toast, { Toaster } from "react-hot-toast"; // 👈 MODERN TOAST NOTIFICATIONS
...
export default function ViewLead({ lead, activeTab }: { lead: any, activeTab?: string }) {
  ...
  const valueStyle = "text-sm font-medium text-slate-900 bg-slate-50/50 px-3 py-2 rounded-md border border-slate-100 min-h-[38px] flex items-center";
  ...
  const addPaymentRow = () => setOtherPayments([...otherPayments, { id: Math.random().toString(36).substr(2, 9), name: "", amount: "", remarks: "", invoice: "", date: "", testDate: "", status: "Unsubmitted", rejectReason: "", isAutoRetest: false, isAutoReschedule: false }]);
  ...
  const removePaymentRow = (id: string) => { 
    setIsDirty(true); 
    setOtherPayments(prev => prev.filter((p: any) => p.id !== id)); 
  };
  ...
  const renderVerifyBadge = (status: string) => {
    if (status === "Approved") return <span className="flex items-center gap-1 text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded border border-emerald-200"><Icons.Check /> Approved</span>;
    if (status === "Pending") return <span className="flex items-center gap-1 text-[11px] font-bold text-amber-700 bg-amber-50 px-2.5 py-1 rounded border border-amber-200 animate-pulse">⏳ Pending HR</span>;
    if (status === "Rejected") return <span className="flex items-center gap-1 text-[11px] font-bold text-rose-700 bg-rose-50 px-2.5 py-1 rounded border border-rose-200"><Icons.Alert /> Rejected</span>;
  ...
  const renderVerifyButton = (status: string, onClick: () => void, loadingId: string, disabledStyle = "bg-slate-100 text-slate-400 border-slate-200") => {
    if (status === "Approved") return <button type="button" disabled className="w-full py-2 bg-emerald-50/50 text-emerald-600 text-sm font-semibold rounded-md border border-emerald-200/50 cursor-not-allowed flex items-center justify-center gap-2"><Icons.Check /> Verified</button>;
    if (status === "Pending") return <button type="button" disabled className="w-full py-2 bg-amber-50/50 text-amber-600 text-sm font-semibold rounded-md border border-amber-200/50 cursor-not-allowed animate-pulse flex items-center justify-center gap-2">⏳ Verification Sent</button>;
    return (
      <button type="button" onClick={onClick} disabled={verifyLoading === loadingId} className={`w-full py-2 text-white text-sm font-semibold rounded-md shadow-sm transition-all hover:shadow focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500 disabled:${disabledStyle} bg-indigo-600 hover:bg-indigo-700 flex items-center justify-center gap-2`}>
  ...
  const renderUploadReceiptButton = (docType: string) => (
    <button 
      ...
      className="w-full py-2 mt-2 bg-white hover:bg-slate-50 text-slate-700 border border-slate-300 text-xs font-semibold rounded-md transition-colors shadow-sm flex justify-center items-center gap-2 focus:ring-2 focus:ring-slate-200"
    >
  ...
  const getExpiryCountdown = (dateString?: string | null) => {
    ...
    if (exp < now) return <span className="text-xs font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded border border-rose-100">Expired</span>;
    ...
    if (m === 0 && d === 0) return <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-100">Expires Today</span>;
    ...
    return (
      <div className="mt-10">
        <div className="bg-slate-50/50 p-6 rounded-xl border border-slate-200/80 shadow-sm mb-6">
          ...
          {/* ⚡ ZERO-LAG TYPING: Inputs now use defaultValue instead of state! */}
          <textarea 
            name={dbName} 
            disabled={currentTab !== prefix} 
            defaultValue={lead[dbName] || ""} 
            rows={4} 
            placeholder={`Add detailed context specifically regarding ${tabTitle}...`} 
            className={inputStyle}
          ></textarea>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-indigo-100 shadow-sm relative overflow-hidden">
          ...
          <h2 className="text-sm font-bold text-indigo-900 border-b border-indigo-100 pb-3 mb-5 flex items-center gap-2">
            <Icons.Refresh /> Global Follow-Ups & Status
          </h2>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input 
                ...
                disabled={currentTab !== prefix} 
                defaultValue={lead.followUpRemarks || ""} 
                placeholder="Brief notes from the last interaction..." 
                className={inputStyle} 
              />
            </div>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto pb-24 pt-4 relative">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-5xl rounded-xl shadow-xl flex flex-col max-h-[90vh] overflow-hidden border border-slate-200">
            <div className="flex justify-between items-center p-4 border-b border-slate-100 bg-slate-50/50 shrink-0">
              <h2 className="text-slate-800 font-bold text-base flex items-center gap-2"><Icons.Folder /> Document Vault</h2>
              <button type="button" onClick={() => setIsVaultModalOpen(false)} className="text-slate-400 hover:text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 w-7 h-7 rounded-full flex items-center justify-center transition-colors shadow-sm"><Icons.Cross /></button>
            </div>
      ...
      {/* ENTERPRISE HEADER WITH SMART LINKS */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-6 gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-slate-500 font-medium">
            ...
            <span className="flex items-center gap-1 group">
              <a href={`tel:${lead.callingNumber}`} className="hover:text-indigo-600 hover:underline">{lead.callingNumber}</a> 
            ...
            {lead.email && (
              <span className="flex items-center gap-1 text-slate-500 group">
                ✉️ <a href={`mailto:${lead.email}`} className="hover:text-indigo-600 hover:underline">{lead.email}</a> 
        ...
        <div className="flex items-center gap-3">
          <Link href="/sales/leads" className="px-4 py-2 rounded-md font-medium text-slate-600 bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2 text-sm">
            <Icons.ArrowLeft /> Back to Leads
          </Link>
        </div>
      </div>

      {/* 🚀 MAIN WRAPPER FORM (OnChange attached here to trigger Floating Save Bar!) */}
      <form ref={formRef} onChange={() => setIsDirty(true)} onSubmit={(e) => handleInlineSave(e, false)}>
            ...
            <div className="sticky top-0 z-40 pt-4 pb-2 bg-slate-50/95 backdrop-blur-md -mx-4 px-4 sm:mx-0 sm:px-0 sm:bg-transparent sm:backdrop-blur-none transition-all">
              <div className="flex overflow-x-auto custom-scrollbar p-1 bg-slate-200/50 backdrop-blur-md rounded-lg shadow-sm border border-slate-200/60">
                ...
                <button 
                  type="button" 
                  onClick={() => { 
                    if (!isExamPassed) return toast.error("Candidate must pass exam to unlock Agreement."); 
                    setCurrentTab("sa"); 
                  }} 
                  className={`flex-1 flex justify-center items-center gap-2 px-3 py-2 text-sm font-medium rounded transition-all duration-200 ${currentTab === 'sa' ? 'bg-white text-emerald-700 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-700 hover:bg-white/50'} ${!isExamPassed ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {isExamPassed ? <Icons.Handshake /> : <Icons.Lock />} Agreement
                </button>
              </div>
            </div>
            ...
            <div className={currentTab === "profile" ? "space-y-5 animate-in fade-in duration-300 block mt-4" : "hidden"}>
              <div className={sectionStyle}>
                <div className="border-b border-slate-100 pb-3 mb-4 flex justify-between items-center">
                  <h2 className="text-base font-bold text-slate-800 flex items-center gap-2">1. Routing Information</h2>
                  <button 
                    type="button" 
                    onClick={() => { 
                      if (isEditingProfile) handleInlineSave(); 
                      else setIsEditingProfile(true); 
                    }} 
                    disabled={loading} 
                    className={`text-xs font-semibold px-3 py-1.5 rounded transition-all shadow-sm disabled:opacity-50 flex items-center gap-1 ${isEditingProfile ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-white text-slate-600 border border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}
                  >
                    {loading && isEditingProfile ? "Saving..." : isEditingProfile ? <><Icons.Check /> Done</> : <><Icons.Edit /> Edit</>}
                  </button>
                </div>
              ...
              <div className={sectionStyle}>
                <h2 className="text-base font-bold text-slate-800 border-b border-slate-100 pb-3 mb-4">2. Client Information</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
              ...
              <div className={sectionStyle}>
                <h2 className="text-base font-bold text-slate-800 border-b border-slate-100 pb-3 mb-4">3. Experience & Agency History</h2>
            ...
            <div className={currentTab === "documents" ? "space-y-5 animate-in fade-in duration-300 block mt-4" : "hidden"}>
              <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                <div className="bg-slate-50/80 px-5 py-4 border-b border-slate-200 flex justify-between items-center">
                  <h2 className="text-sm font-bold text-slate-800 flex items-center gap-2"><Icons.Folder /> Core ID Checklist</h2>
              ...
              <div className="mt-6 border-t border-slate-200 pt-6">
                <h2 className="text-base font-bold text-slate-800 mb-4 flex items-center gap-2"><Icons.Folder /> Complete Document Vault</h2>
                <DocumentVault 
                  leadId={lead.id} 
                  existingDocs={lead.documentFiles} 
                  defaultCategory={vaultDefaultCategory} 
              ...
              {combinedHistory.length > 0 && (
                <div className="bg-slate-50 p-5 rounded-lg border border-slate-200 mb-6">
                  <h2 className="text-base font-bold text-slate-800 border-b border-slate-200 pb-3 mb-5 flex justify-between items-center">
                    ...
                    <span className="text-[10px] font-medium text-slate-500 bg-white border border-slate-200 px-2 py-0.5 rounded flex items-center gap-1"><Icons.Lock /> Read-Only</span>
                  </h2>
                  ...
                  <div className="space-y-5">
                    <div>
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className={`text-[11px] font-bold px-2.5 py-1 rounded border shadow-sm ${lead.examinerStatus === "Approved" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : (lead.examinerStatus === "Denied" || lead.examinerStatus === "Rejected") ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-white text-slate-600 border-slate-200"}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div className="bg-white p-4 rounded-md border border-slate-200 shadow-sm flex flex-col items-center text-center">
                          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">English Assessment</p>
                          <p className="text-3xl font-bold text-slate-800 tracking-tight">{lead.englishScore !== null ? lead.englishScore : "-"}<span className="text-sm text-slate-300 font-medium ml-1">/10</span></p>
                          {renderProgressBar(lead.englishScore, lead.englishTestResult)}
                          <p className={`text-xs font-bold mt-2 ${lead.englishTestResult === 'Passed' ? 'text-emerald-600' : lead.englishTestResult === 'Failed' ? 'text-rose-600' : 'text-slate-400'}`}>{lead.englishTestResult || "Pending"}</p>
                        </div>
                        <div className="bg-white p-4 rounded-md border border-slate-200 shadow-sm flex flex-col items-center text-center">
                          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Driving / Yard Test</p>
                          <p className="text-3xl font-bold text-slate-800 tracking-tight">{lead.drivingScore !== null ? lead.drivingScore : "-"}<span className="text-sm text-slate-300 font-medium ml-1">/10</span></p>
                          {renderProgressBar(lead.drivingScore, lead.yardTestResult)}
                          <p className={`text-xs font-bold mt-2 ${lead.yardTestResult === 'Passed' ? 'text-emerald-600' : lead.yardTestResult === 'Failed' ? 'text-rose-600' : 'text-slate-400'}`}>{lead.yardTestResult || "Pending"}</p>
                        </div>
                      </div>
                      
                      {lead.examinerRemarks && (
                        <div className="bg-white p-4 rounded-md border border-slate-200 shadow-sm">
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Latest Examiner Remarks</p>
                          <p className="text-xs font-medium text-slate-700 whitespace-pre-wrap">{lead.examinerRemarks}</p>
                        </div>
                      )}
                    </div>

                    <div className="pt-4 border-t border-slate-200">
                      <h3 className="text-[11px] font-bold text-slate-500 mb-3 uppercase tracking-wider">Testing History ({combinedHistory.length} Attempts)</h3>
                      <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                        {combinedHistory.map((test: any) => (
                          <div key={test.id} className={`p-3 rounded-md border flex justify-between items-center ${test.isMissed ? 'bg-amber-50/50 border-amber-200' : 'bg-white border-slate-200 shadow-sm'}`}>
                            <div>
                              <p className={`text-xs font-bold ${test.isMissed ? 'text-amber-800' : 'text-slate-800'}`}>{test.attemptLabel} <span className={`font-medium text-[10px] ml-1.5 ${test.isMissed ? 'text-amber-600' : 'text-slate-400'}`}>({new Date(test.createdAt).toLocaleDateString("en-GB")})</span></p>
                              <p className="text-[11px] text-slate-500 mt-1 font-medium">
                                English: <span className={`font-semibold ${test.isMissed ? 'text-amber-600' : 'text-slate-700'}`}>{test.englishScore !== "-" ? `${test.englishScore}/10` : "-"}</span> <span className={test.isMissed ? "text-amber-500" : ""}>({test.englishTestResult})</span> <span className="mx-1.5 text-slate-200">|</span> 
                                Yard: <span className={`font-semibold ${test.isMissed ? 'text-amber-600' : 'text-slate-700'}`}>{test.drivingScore !== "-" ? `${test.drivingScore}/10` : "-"}</span> <span className={test.isMissed ? "text-amber-500" : ""}>({test.yardTestResult})</span>
                              </p>
                            </div>
                            <span className={`px-2 py-1 text-[9px] font-bold rounded uppercase shrink-0 shadow-sm border ${test.isMissed ? 'bg-amber-100 text-amber-700 border-amber-200' : test.status === 'Approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>{test.status}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="mb-8">
                  ...
                  {lead.testFeeVerifyStatus === 'Rejected' && lead.testFeeRejectReason && (
                    <div className="mb-4 p-3 bg-rose-50 border-l-2 border-rose-500 rounded-r shadow-sm">
                      <strong className="block uppercase tracking-wider text-[9px] text-rose-700 mb-0.5 font-bold flex items-center gap-1"><Icons.Alert /> HR Rejection Reason:</strong>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-50 p-4 rounded-md border border-slate-200 flex flex-col justify-between">
                      <div>
                        <h4 className="text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-3 flex items-center gap-1.5"><Icons.Calendar /> Schedule Date</h4>
                        <div className="mb-4">
                          <label className={labelStyle}>Test Date {isInitialDateLocked && <Icons.Lock />}</label>
                          <input 
                            type="date" 
                            name="testDate" 
                            disabled={currentTab !== "testing"} 
                    ...
                    <div className="bg-slate-50 p-4 rounded-md border border-slate-200 flex flex-col justify-between">
                      <div>
                        <div className="flex justify-between items-center mb-3">
                          <h4 className="text-[11px] font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5"><Icons.Payment /> Collect Payment</h4>
                      ...
                      <div className="space-y-2">
                        ...
                        {renderUploadReceiptButton("Initial Test Receipt")}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="mb-8 animate-in fade-in slide-in-from-top-4 pt-6 border-t border-slate-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-slate-50 p-4 rounded-md border border-slate-200 flex flex-col justify-between hover:border-slate-300 transition-colors">
                        <div>
                          <h4 className="text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-3 flex items-center gap-1.5"><Icons.Calendar /> Schedule Re-Test</h4>
                          <div className="mb-4">
                            <label className={labelStyle}>Re-Test Date {isRetestDateLocked && <Icons.Lock />}</label>
                            <input 
                              type="date" 
                              name="reTestDate" 
                              disabled={currentTab !== "testing"} 
                              ...
                              className={isRetestDateLocked ? lockedInputStyle : inputStyle} 
                      ...
                      <div className="bg-slate-50 p-4 rounded-md border border-slate-200 flex flex-col justify-between hover:border-slate-300 transition-colors">
                          ...
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            <div>
                              ...
                              <input 
                                ...
                                placeholder="0.00" 
                                className={lead.reTestFeeVerifyStatus === 'Pending' || lead.reTestFeeVerifyStatus === 'Approved' ? lockedInputStyle : inputStyle} 
                        ...
                        <div className="space-y-2 mt-4">
                          {renderVerifyButton(lead.reTestFeeVerifyStatus || "Unsubmitted", () => sendForVerification('RETEST'), "RETEST", "bg-slate-100 text-slate-400")}
                          {renderUploadReceiptButton("Re-Test Receipt")}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  return (
                    <div key={payment.id} className="mb-8 animate-in fade-in slide-in-from-top-4 pt-6 border-t border-slate-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-slate-50 p-4 rounded-md border border-slate-200 flex flex-col justify-between hover:border-slate-300 transition-colors">
                          <div>
                            <h4 className="text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-3 flex items-center gap-1.5"><Icons.Calendar /> Schedule {payment.isAutoReschedule ? "Reschedule" : "Re-Test"}</h4>
                            <div className="mb-4">
                              <label className={labelStyle}>New Date {isDynamicDateLocked && <Icons.Lock />}</label>
                              <input 
                                type="date" 
                                ...
                                className={isDynamicDateLocked ? lockedInputStyle : inputStyle} 
                        ...
                        <div className="bg-slate-50 p-4 rounded-md border border-slate-200 flex flex-col justify-between hover:border-slate-300 transition-colors">
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-3">
                              <div>
                                ...
                                <input 
                                  ...
                                  placeholder="0.00" 
                                  className={isPendingOrApproved ? lockedInputStyle : inputStyle} 
                          ...
                          <div className="space-y-2 mt-4">
                            {renderVerifyButton(payment.status || "Unsubmitted", () => sendOtherForVerification(payment.id), payment.id, "bg-slate-100 text-slate-400")}
                            {renderUploadReceiptButton(`Misc Receipt - ${payment.name}`)}
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className={currentTab === "sa" ? "space-y-5 animate-in fade-in duration-300 block mt-4" : "hidden"}>
              <div className="mb-8 p-6 bg-white rounded-lg shadow-sm border border-slate-200 relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row md:justify-between md:items-center border-b border-slate-100 pb-4 mb-5">
                  <h3 className="text-base font-bold text-slate-800 flex items-center gap-2"><Icons.Handshake /> Service Agreement Processing</h3>
                  <div className="flex items-center gap-3 mt-3 md:mt-0">
                    {renderVerifyBadge(lead.saFeeVerifyStatus)}
                    {!isExamPassed ? (
                      <span className="text-[10px] font-bold bg-slate-50 text-slate-500 px-3 py-1.5 rounded-md border border-slate-200 shadow-sm flex items-center gap-1.5"><Icons.Lock /> Locked</span>
                ...
                {!isExamPassed && (
                  <div className="mb-5 p-4 bg-amber-50 border border-amber-200 rounded-md shadow-sm flex items-start gap-3">
                    <div className="text-amber-500 mt-0.5"><Icons.Alert /></div>
                    <div>
                      <strong className="block text-[11px] font-bold text-amber-800 mb-0.5">Agreement Locked</strong>
                      <span className="text-xs text-amber-700 font-medium">Cannot process until candidate passes exam.</span>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-5 bg-slate-50 p-5 rounded-md border border-slate-200">
                  <div>
                    ...
                    <input 
                      ...
                      disabled={currentTab !== "sa"} 
                      defaultValue={lead.serviceAgreementAmount} 
                      placeholder="0.00" 
                      className={!isExamPassed || lead.saFeeVerifyStatus === 'Pending' || lead.saFeeVerifyStatus === 'Approved' ? lockedInputStyle : inputStyle} 
                  ...
                  <div className="md:col-span-4 mt-2">
                    {renderUploadReceiptButton("Service Agreement Receipt")}
                  </div>
                </div>
              </div>
