      ...
      <div className="flex space-x-2 border-b border-slate-200 px-2">
        <Link 
          ...
          className={`px-6 py-3.5 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${
            currentView === 'action' ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 hover:text-slate-700'
          }`}
        >
          ⚡ HR Action Required
          <span className={`px-2 py-0.5 rounded-full text-[10px] ${currentView === 'action' ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>
        ...
        <Link 
          ...
          className={`px-6 py-3.5 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${
            currentView === 'waiting' ? 'border-slate-800 text-slate-800' : 'border-transparent text-slate-500 hover:text-slate-700'
          }`}
        >
          ⏳ Waiting on Operations
          <span className={`px-2 py-0.5 rounded-full text-[10px] ${currentView === 'waiting' ? 'bg-slate-200 text-slate-800' : 'bg-slate-100 text-slate-500'}`}>
        ...
        {currentView === "action" ? (
                ...
                <>
                  {followUpsDue.length > 0 && (
                    <div className="bg-white rounded-xl shadow-sm border-2 border-orange-300 overflow-hidden animate-in fade-in duration-300">
                      <div className="p-4 bg-orange-50 border-b border-orange-200 flex justify-between items-center">
                        <h2 className="font-bold text-orange-900 flex items-center gap-2">🔔 Action Required: Follow-Ups Due</h2>
                        <span className="text-xs font-bold bg-orange-200 text-orange-800 px-3 py-1 rounded-full uppercase tracking-wider">{followUpsDue.length} Scheduled</span>
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border-2 border-blue-200 overflow-hidden animate-in fade-in duration-300">
                    <div className="p-4 bg-blue-50 border-b border-blue-100 flex justify-between items-center">
                      <h2 className="font-bold text-blue-900 flex items-center gap-2">⚡ Priority Processing Queue</h2>
                    </div>
                          ...
                          {standardQueue.length === 0 ? (
                            <tr>
                              <td colSpan={session.user.branch === "MASTER" ? 6 : 5} className="p-16 text-center">
                                <div className="flex flex-col items-center justify-center">
                                  ...
                                  <h3 className="text-slate-700 font-bold text-lg">{searchTerm ? "No matches found" : "You're all caught up!"}</h3>
                                  <p className="text-slate-500 mt-1">{searchTerm ? "Try searching by a different name or ID." : "No standard files currently require direct HR action."}</p>
                                </div>
                              </td>
                            </tr>
                          ) : (
                            standardQueue.map((lead) => (
                              <tr key={lead.id} className="hover:bg-blue-50/30 transition-colors bg-white">
                                ...
                                <td className="p-4">
                                  <span className={`px-3 py-1 text-xs font-bold rounded-full ${lead.caseStatus.includes("Visa") ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}>
                                    {lead.caseStatus}
                                  </span>
                                </td>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in duration-300">
            <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <h2 className="font-bold text-slate-700 flex items-center gap-2">⏳ Delegated to Operations</h2>
            </div>
                  ...
                  {waitingOnOpsQueue.length === 0 ? (
                    <tr>
                      <td colSpan={session.user.branch === "MASTER" ? 6 : 5} className="p-16 text-center">
                        <div className="flex flex-col items-center justify-center">
                          <span className="text-4xl mb-4">{searchTerm ? "🕵️‍♂️" : "🎉"}</span>
                          <h3 className="text-slate-700 font-bold text-lg">{searchTerm ? "No matches found" : "Operations is all caught up!"}</h3>
                        </div>
                      </td>
                    </tr>
                  ) : (
